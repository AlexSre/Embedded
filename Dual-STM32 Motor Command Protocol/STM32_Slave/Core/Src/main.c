...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  uint8_t calculate_crc = packet.start ^ packet.command ^ packet.left_pwm ^ packet.left_pwm ;

	  if(packet.start == START_BYTE && packet.command == CMD_SET_PWM && packet.crc == calculate_crc && packet.left_pwm <= 100 && packet.right_pwm <= 100 ){
